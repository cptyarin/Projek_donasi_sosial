    ...
    <div class="col-lg-8">
        <div class="card-mokapos">
            <div class="card-header">
                <i class="fas fa-edit me-2 text-warning"></i> Edit Penyaluran Bantuan
            </div>
            <div class="card-body">
                <form action="{{ route('distributions.update', $distribution) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select form-control-mokapos @error('penerima_id') is-invalid @enderror" name="penerima_id">
                            @foreach($recipients as $r)
                                <option value="{{ $r->id }}" {{ old('penerima_id', $distribution->penerima_id) == $r->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select form-control-mokapos @error('program_id') is-invalid @enderror" name="program_id">
                            @foreach($programs as $prog)
                                <option value="{{ $prog->id }}" {{ old('program_id', $distribution->program_id) == $prog->id ? 'selected' : '' }}>
                                    {{ $prog->nama_program }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" step="0.01" class="form-control form-control-mokapos @error('nominal') is-invalid @enderror" 
                               name="nominal" value="{{ old('nominal', $distribution->nominal) }}" placeholder="Masukkan nominal bantuan" min="0">
                        @error('nominal')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control form-control-mokapos @error('tanggal_penyaluran') is-invalid @enderror" 
                               name="tanggal_penyaluran" value="{{ old('tanggal_penyaluran', $distribution->tanggal_penyaluran->format('Y-m-d')) }}">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control form-control-mokapos @error('keterangan') is-invalid @enderror" 
                                  name="keterangan" rows="3" placeholder="Catatan tambahan tentang penyaluran (opsional)">{{ old('keterangan', $distribution->keterangan) }}</textarea>
                        @error('keterangan')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
